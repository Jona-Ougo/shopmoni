...
class GoCart {
    ...
    public function insertItem($data=[])
    {
        ...
        $subTotal   = format_currency($this->getSubtotal());

        if($update)
        {
            return json_encode(['message'=>lang('cart_updated'), 'itemCount'=>$itemCount, 'subtotal'=>$subTotal]);
        }
        else
        {
            return json_encode(['message'=>lang('item_added_to_cart'), 'itemCount'=>$itemCount, 'subtotal'=>$subTotal]);
        }
    }
