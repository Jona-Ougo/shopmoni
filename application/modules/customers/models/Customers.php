...
Class Customers extends CI_Model
{
    ...
    public function get_merchants($limit=0, $offset=0, $order_by='id', $direction='DESC')
    {
        CI::db()->where('use_shop', 1)->order_by($order_by, $direction);
        if($limit>0)
        {
            CI::db()->limit($limit, $offset);
        }
    ...
    public function reduce_advert_credit($customerId, $advertCredit)
    {
        $customer = $this->get_customer($customerId);

        CI::db()->where('id', $customer->id);

        $newAdvertCredit = intval($customer->advert_credit) - intval($advertCredit);
        $insertParams = [
            'advert_credit' => $newAdvertCredit
        ];

        CI::db()->update('customers', $insertParams);
    }

    //Returns users who has ordered products posted by the loggedin user.
    public function getShopCustomers($limit=0, $offset=0, $order_by='id', $direction='DESC')
    {
        \CI::load()->model('Orders');
        $customer_ids = [];
        $merchantOrders = \CI::Orders()->getShopOrders();
        foreach($merchantOrders as $row)
            $customer_ids[] = $row->customer_id; 
        //$customer_ids = array_unique($customer_ids);

        //If there are no customers who has once ordered products posted by $this->customer return empty array.
        if(count($customer_ids) < 1)
            return [];
        
        CI::db()->where('is_guest', 0)->where_in('id', $customer_ids)->order_by($order_by, $direction);
    ...
    //Returns the count of a merhcant's customers.
    public function countShopCustomers()
    {
        $customer_ids = [];
        $shop_customer_ids_count = 0;

        if(\CI::session()->userdata('shop_customer_ids_count') == ''){
            \CI::load()->model('Orders');
            $merchantOrders = \CI::Orders()->getShopOrders();
            foreach($merchantOrders as $row)
                $customer_ids[] = $row->customer_id; 
            $customer_ids = array_unique($customer_ids);
            \CI::session()->set_userdata('shop_customer_ids_count', count($customer_ids));
        }else{
            $customer_ids = \CI::session()->userdata('shop_customer_ids_count');
        }
        
        return $shop_customer_ids_count;
    }
    ...
    public function loginAudit(){
        ...
        CI::db()->select('customers.firstname, customers.lastname, login_audit.*');
        CI::db()->join('customers', 'customers.id = login_audit.customer_id');
        CI::db()->limit(100)->order_by('login_audit.created_at', 'DESC');
        return CI::db()->get('login_audit')->result();
    }
}
