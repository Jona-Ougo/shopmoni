...
class Search extends Front {
    ...
    public function index($code=false, $sort='name', $dir="ASC", $page = 0)
    {
        ...
        if(!$code)
        {
            ...
            $category_id    = \CI::input()->post('category_id', true);
            if(empty($term))
            {
            ...
            {
                $code = \CI::Search()->recordTerm($term, $category_id);

                // no code? redirect so we can have the code in place for the sorting.
                // I know this isn't the best way... (Man! just get the job done -Segun)
        ...
        else
        {
            ...
            $row            = \CI::Search()->getTerm($code);
            $term           = $row->term;
            $category_id    = $row->category_id == '' ? false : $row->category_id;
        }
        ...
        else
        {
            ...
            $result = \CI::Products()->search_products($term, $per_page, $page, $sort, $dir, $category_id);
