...
class AdminProducts extends Admin {
    ...
    public function index($rows=100, $order_by="name", $sort_order="ASC", $code=0, $page=0)
    {
        ...
        $data['products'] = \CI::Products()->products(array('term'=>$term, 'order_by'=>$order_by, 'sort_order'=>$sort_order, 'rows'=>$rows, 'page'=>$page), false, $this->customer->id);

        //total number of products
        $data['total'] = \CI::Products()->products(array('term'=>$term, 'order_by'=>$order_by, 'sort_order'=>$sort_order), true, $this->customer->id);
    ...
    public function product_autocomplete()
    {
        ...
        if(empty($name)){
            ...
            $results = \CI::Products()->product_autocomplete_shop($name, $limit);
    ...
    public function giftCardForm($id = false, $duplicate = false)
    {
        ...
        */
        ...
        else
        {
            ...
            $save['id']         = $id;
            $save['sku']        = get_sku(\CI::input()->post('name'));
            $save['name']       = \CI::input()->post('name');
            $save['seo_title']  = \CI::input()->post('seo_title');
            $save['meta']       = \CI::input()->post('meta');
            $save['description'] = \CI::input()->post('description');
            ...
            // save product
            $product_id = \CI::Products()->save($save, $options, $categories, $this->customer_id);
