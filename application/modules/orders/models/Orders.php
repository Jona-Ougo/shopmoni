...
Class Orders extends CI_Model
{
    ...
    public function getOrders($search=false, $sort_by='', $sort_order='DESC', $limit=0, $offset=0)
    {
        ...
        if ($search)
        {
            ...
            if(!empty($search->end_date))
            {
                ...
                //I tried <= but it did not public function. Any ideas why? I dont know too #massive.
    ...
    //Returns orders placed on customer's shop.
    public function getShopOrders($search=false, $sort_by='', $sort_order='DESC', $limit=0, $offset=0)
    {
        $select = 'parent_orders.*'.$this->getAddressSelect();
        $customer = \CI::Login()->customer();
        
        \CI::db()->select('order_number, ordered_on, shipped_on')
        ->join('orders as parent_orders', 'parent_orders.id = order_items.order_id');
        \CI::db()->select($select)
        ->join('customers_address_bank as shipping', 'shipping.id = parent_orders.shipping_address_id', 'left');
        \CI::db()->join('customers_address_bank as billing', 'billing.id = parent_orders.billing_address_id', 'left');
        \CI::db()->where('order_items.customer_id', $customer->id);
        ...
        if($search){
            ...
            if(!empty($search->start_date)){
                CI::db()->where('parent_orders.ordered_on >=',$search->start_date);
            }
            if(!empty($search->end_date)){
                //increase by 1 day to make this include the final day
                //I tried <= but it did not public function. Any ideas why? I dont know too #massive.
                $search->end_date = date('Y-m-d', strtotime($search->end_date)+86400);
                CI::db()->where('parent_orders.ordered_on <',$search->end_date);
            }
        }
        ...
        //This helps me group and also get the total value of current customer's share in each order.
        CI::db()->where('parent_orders.status !=', 'cart');
        CI::db()->select('SUM(order_items.total_price * order_items.quantity) as customer_total');
        CI::db()->select('COUNT(*) as count');
        CI::db()->select('item_status');
        CI::db()->select('order_items.customer_id as merchant_id');
        CI::db()->group_by('order_items.order_id');
        return CI::db()->select('order_id')->get('order_items')->result();
    }
    ...
    public function getShopOrderCount($search=false)
    {
        $customer = \CI::Login()->customer();

        \CI::db()->join('orders as parent_orders', 'parent_orders.id = order_items.order_id');
        \CI::db()->join('customers_address_bank as shipping', 'shipping.id = parent_orders.shipping_address_id', 'left');
        \CI::db()->join('customers_address_bank as billing', 'billing.id = parent_orders.billing_address_id', 'left');
        \CI::db()->where('order_items.customer_id', $customer->id);
        ...
        CI::db()->where('parent_orders.status !=', 'cart');
        CI::db()->group_by('order_items.order_id');
        return CI::db()->get('order_items')->num_rows();
        //return CI::db()->count_all_results('order_items');
    }
    ...
    //customer id here refers to the current logged in customer not the person who ordered.
    public function getItems($id, $customer_id = 0, $joinOwner = false)
    {
        if($customer_id == 0){
            CI::db()->where('order_id', $id)->order_by('type', 'ASC')->order_by('order_items.id', 'ASC');
        }else{
            $where = ['order_id'=>$id, 'customer_id'=>$customer_id];
            CI::db()->where($where)->order_by('type', 'ASC')->order_by('order_items.id', 'ASC');
        }
        
        //We add the record of the owner of the product too. used in zeus/orders
        if($joinOwner){
    ...
    public function saveOrder($data, $contents = false)
    {
        ...
        if($contents)
        {
            ...
            foreach($contents as $item)
            {
                ...
                //Also store the product's owner's id.
                $save['customer_id']    = \DB\DB::getCell('products', ['id'=>$save['product_id']], 'customer_id');
                $save['quantity']       = $item['quantity'];
                $save['order_id']       = $id;
                $save['created_at']     = dateI('Y-m-d H:i:s');
