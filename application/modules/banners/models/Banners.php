...
Class Banners extends CI_Model
{
    ...
    public function save_customer_banner($data, $customerId)
    {
        $banner_collection_id = $data['banner_collection_id'];

        $bannerCollections = DB\DB::tableJoin('banner_collections', 'advert_credit_id', ['advert_credit'], ['banner_collection_id'=>$banner_collection_id]);
        $customer = \CI::Customers()->get_customer($customerId);

        if ($customer->advert_credit < $bannerCollections[0]->value) {
            return ['status' => false, 'message' => 'Your Advert Credit Point is not enough'];
        } else {
            $response =  CI::db()->insert('banners', $data);

            if ($response) {

                \CI::Customers()->reduce_advert_credit($data['customer_id'],$bannerCollections[0]->value);

                return ['status'=> true, 'message'=> 'Advert created successfully'];
            }

            return ['status' => false, 'message' => 'Error creating advert. Please contact support'];
        }
    }


    public function banner_collection_input() {
        $bannerCollections = $this->banner_collections();
        $bannerCollectionsInput = [];

        foreach ($bannerCollections as $bannerCollection) {
            $bannerCollectionsInput[$bannerCollection->banner_collection_id] = $bannerCollection->name;
        }

        return $bannerCollectionsInput;

    }

    public function banner_adverts()
    {
        $data['page_title'] = lang('advert_orders');

        $data['adverts'] = DB\DB::tableJoin('banners', 'id', ['customers'], []);
