...
<div class="row">
    <div class="col-md-8">
        ...
        <div class="form-group">
            <label for="image"><?php echo lang('image');?> <em class="text-danger">Recommended size: 850px X 230px</em></label>
    ...
    <div class="col-md-4">
        ...
        <div class="form-group">
            <label for="slug">Filter</label>
            <?php $selected = $filter_id < 1 ? set_value('filter_id') : $filter_id ?>

            <select name="filter_id" class="form-control">
                <option value="0">--NOT APPLICABLE--</option>
                <?php foreach(\DB\DB::get('category_filters') as $row): ?>
                    <?php $cat = \DB\DB::getRow('categories', ['id'=>$row->category_id]) ?>
                    <?php $text = $row->name.' <small>('.$cat->name.')</small>' ?>
                    <?php if($row->filter_id == $selected): ?>
                        <option value="<?=$row->filter_id ?>" selected><?=$text ?></option>
                    <?php else: ?>
                        <option value="<?=$row->filter_id ?>"><?=$text ?></option>
                    <?php endif; ?>
                <?php endforeach;; ?>
            </select>
        </div>
